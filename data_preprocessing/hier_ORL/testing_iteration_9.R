...
count_NA_X_outcome <- data.frame()
count_NA_x_choice <- data.frame()
...
for (i in 1:niterations) {
  ...
  if (sum(is.na(X))>0) {
    # fixing outcome
    sum_NA = sum(is.na(X))
    print(c(sum_NA, "NAs found"))
    temp = data.frame(i, sum_NA)
    count_NA_X_outcome <- rbind(count_NA_X_outcome, temp)# add them to df
    X[is.na(X)] <- 0 # replace NA's
    sum_NA = sum(is.na(X))
    print(c(sum_NA, " NAs found after replacement")) # print the number of NA's
  }
  ...
  if (sum(is.na(x))>0) {
    ...
    print(c(sum_NA, " NAs found"))
    temp = data.frame(i, sum_NA)
    count_NA_x_choice <- rbind(count_NA_x_choice, temp)  # add them to df
    x[is.na(x)] <- round(runif(sum(is.na(x)), min = 1, max = 4)) # replace NA's
    sum_NA = sum(is.na(x))
    print(c(sum_NA, " NAs found after replacement")) # print the number of NA's
  }
