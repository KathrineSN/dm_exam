...
#df_final <- rbind(trials_95, trials_100, trials_150)
df_final <- trials_100
```
...
par(mfrow=c(2,2))
plot(density(temp_samples$BUGSoutput$sims.list$w), main = "w")
plot(density(temp_samples$BUGSoutput$sims.list$A), main = "A")
plot(density(temp_samples$BUGSoutput$sims.list$theta), main = expression(theta))
plot(density(temp_samples$BUGSoutput$sims.list$a), main = "a")
mtext("Posterior distributions",                   # Add main title
      side = 3,
      line = - 1,
      outer = TRUE)
```
...
```{r}
pred_success_adjust <- pred_success/ntrials_all[1:10]
